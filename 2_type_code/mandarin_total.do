...
save "lang_adv_electives.dta", replace
...
save "mandarin.dta", replace

/*
//******* 國學常識 *******//
gen zh_know = 1 if ustrregexm(subj_ep4, "國學常識") == 1
//******* 語文表達與傳播應用 *******//
gen zh_exp = 1 if ustrregexm(subj_ep4, "語文表達與傳播應用") == 1
//******* 各類文學選讀 *******//
gen zh_lit = 1 if ustrregexm(subj_ep4, "各類文學選讀") == 1
//******* 專題閱讀與研究 *******//
gen zh_topic = 1 if ustrregexm(subj_ep4, "專題閱讀與研究") == 1

//學校開課次數
bysort sch_ep5: egen zh_know_sch_nb = max(zh_know_stdnt_times)
bysort sch_ep5: egen zh_exp_sch_nb = max(zh_exp_stdnt_times)
bysort sch_ep5: egen zh_lit_sch_nb = max(zh_lit_stdnt_times)
*/
...
save "result_2\學校國文總開課數分布(學分數)_0902_pre.dta", replace

use "all_stdnt_nb.dta", clear
merge 1:1 sch using "result_2\學校國文總開課數分布(學分數)_0902_pre.dta"
...
//******* 各校國文修過人數分布 *******//
use "all_stdnt_nb.dta", clear
merge 1:m sch_ep5 using "mandarin.dta"
drop _merge

keep sch_ep5 stdnt_nb id_nat_ncnu_ep5 zh_know_stdnt_cr zh_exp_stdnt_cr zh_lit_stdnt_cr zh_topic_stdnt_cr

duplicates drop sch_ep5 stdnt_nb id_nat_ncnu_ep5 zh_know_stdnt_cr zh_exp_stdnt_cr zh_lit_stdnt_cr zh_topic_stdnt_cr, force
...
save "result_2\各校國文修過人數分布_0902_pre.dta", replace
...
keep sch_ep5 id_nat_ncnu_ep5 stdnt_nb pass
...
save "result_2\至少修超過四學分國文加深加廣的各校學生人數及占比_0902_pre.dta", replace
restore

//calculate average credit of Mandarin
keep sch_ep5 id_nat_ncnu_ep5 stdnt_nb zh_elect_cr
...
save "result_2\國文加深加廣各校學生平均修課學分數_0902_pre.dta", replace

/*
//******* 各校國文修課人次分布 *******//
keep sch_ep5 id_nat_ncnu_ep5 zh_know_stdnt_times zh_exp_stdnt_times zh_lit_stdnt_times zh_topic_stdnt_times

duplicates drop sch_ep5 sch_stdnt_n id_nat_ncnu_ep5 zh_know_times zh_exp_times zh_lit_times zh_topic_times, force

bysort sch_ep5: egen zh_know_times_sch = total(zh_know_times)
bysort sch_ep5: egen zh_exp_times_sch = total(zh_exp_times)
bysort sch_ep5: egen zh_lit_times_sch = total(zh_lit_times)
bysort sch_ep5: egen zh_topic_times_sch = total(zh_topic_times)
*/